...
describe('Full Stack Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        console.log('Service Key Start:', config.supabase.serviceRoleKey?.substring(0, 10));
        console.log('-------------------------');
        server = await createServer(config);
        await server.ready();

        // Verify connection to real Supabase
        const { data: health, error: healthError } = await server.db.service.from('profiles').select('count', { count: 'exact', head: true });

        if (healthError) {
            console.warn('Skipping integration tests: Cannot connect to Supabase.', JSON.stringify(healthError));
            throw new Error(`Supabase connection failed: ${JSON.stringify(healthError)}`);
        }
    ...
    it('GET /me/profile returns profile after manual creation', async () => {
        ...
        const { error: insertError } = await server.db.service.from('profiles').upsert({
            id: userId,
    ...
    it('PUT /me/profile updates the profile', async () => {
        ...
        const { data } = await server.db.service.from('profiles').select('*').eq('id', userId).single();
